      ...
      <Button className="flex items-center justify-start gap-4 bg-transparent shadow-none text-black hover:bg-transparent">
        <div>
          <Avatar>
            ...
            <AvatarFallback className="border">{user.name.charAt(0)}</AvatarFallback>
          </Avatar>
        </div>
        {/* name and last chat */}
        <div className="flex flex-col items-start">
          ...
          <div className="flex flex-row justify-between gap-4">
            <p className="text-xs text-muted-foreground truncate w-40 text-start">{lastMessage ? lastMessage.message : ''}</p>
            <span className="bg-green-500 rounded-full w-3 h-3 text-[8px] text-center flex justify-center items-center text-white">{Math.round(Math.random() * 10) + 1}</span>
          </div>
        </div>
      </Button>
      <ActionMenu />
